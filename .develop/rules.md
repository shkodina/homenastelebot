...
- Не даёт произвольный bash в меню. Shell на хосте — только Cursor → NAS после кнопки Выполнить, через хостовый `agent`, не из контейнера.
...
- Cursor Use — облачный агент **без репозитория**. Cursor NAS — локальный CLI на хосте.
